...
def parse_reports(self):
    ...
    for f in self.find_log_files("sentieon/insertsize", filehandles=True):
        try:
            if  int(os.popen('wc -l {0}/{1}'.format(f['root'], f['fn'])).readline().rstrip().split(' ')[0]) < 6:
                print('SentINS: SKIP', f['fn'])
                continue
        except Exception as e:
            print("There was an error testing for Sentieon Picard INS Metrics, fix here if it's fatal", e)
